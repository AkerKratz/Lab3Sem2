...
#pragma endregion
	...
	private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
		try
		{
			...
			AB.x = -A.x + B.x;
			AB.y = -A.y + B.y;
			AB.length = sqrt(pow(AB.x, 2) + pow(BA.y, 2));

			AC.x = -A.x + C.x;
			AC.y = -A.y + C.y;
			AC.length = sqrt(pow(AC.x, 2) + pow(CA.y, 2));

			if (whatChecked)
			{
				double p = (BA.length + BC.length + CA.length) / 2;
				double bis_b = (2*sqrt(BC.length * BA.length * p*(p- AC.length)))/(BC.length + BA.length);
				Result->Text = Convert::ToString(bis_b);
			}
			else {
				double cosB = (BC.x * BA.x + BC.y * BA.y) / (BC.length * BA.length);
				double cosA = (AC.x * AB.x + AC.y * AB.y) / (AC.length * AB.length);
				double sinB = (1 - pow(cosB, 2));
				double sinA = (1 - pow(cosA, 2));
				double sin = (sinA * cosB) + (cosA * sinB);
				Result->Text = Convert::ToString(sin);
			}
		}
		catch (Exception^ ex)
		{
...
private: System::Void Find_Button_Click(System::Object^ sender, System::EventArgs^ e) {
	try
	{
		...
		if (X==X3)
		{
			...
			double result = Y2 + ((X - X1) / (X3 - X1)) * (Y2 - Y1);
			MessageBox::Show(Convert::ToString(result));
		}
		else if (X1<=X && X<=X2)
		{
			double result = Y1 + ((X -X1) / (X2 - X1)) * (Y2 + Y3);
			MessageBox::Show(Convert::ToString(result));
		}
		else {
			MessageBox::Show("NO");
...
private: System::Void TabulateButton_Click(System::Object^ sender, System::EventArgs^ e) {
	try
	{
		...
		double a = 0, b = 1.5, x = a;
		listBox1->Items->Clear();
		while (x <= b) {
			x += (b - a) / N;
			listBox1->Items->Add(Convert::ToString(sin(2*x)));
		}
	}
	catch (Exception^ ex)
	{
